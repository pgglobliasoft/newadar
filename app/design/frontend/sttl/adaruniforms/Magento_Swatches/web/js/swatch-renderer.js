...
define([
    ...
    $.widget('mage.SwatchRenderer', {
            ...
            $.each(config.options, function () {
                ...
                if (type === 0) {
                    ...
                    html += '<div class="' + optionClass + ' image" ' + attr +
                        ' style="background: url(' + thumb + ') no-repeat center; background-size: 105%;">' + '' +
                        '</div>';
                } else if (type === 3) {
                    // Clear
        ...
        _OnClick: function ($this, $widget, eventName) {
            ...
            if ($this.hasClass('selected')) {
                ...
                if(temp_config_content.length > 0){
                    $('.product-info-main').find('.product.attribute.description .value').html(temp_config_content[0].description);
                    $('.product-info-main').find('.productQuality.product.feature').html(temp_config_content[0].feature);
                    $('.product-info-main').find('.product.attribute.bulletsdetails .value').html(temp_config_content[0].details);
                    $('.product-info-main').find('.fabriccontent-subdiv .value').html(temp_config_content[0].fabric);
                    $('.column.main').find('.fabriccarepopupContainer .modal-content img.fabric_pop_img').attr("src",temp_config_content[0].fabricimg);
                }
            } else {
                if(this._getAttributeCodeById(attributeId) == 'color'){
                    $('.swatch-attribute.seasonalcolors').removeAttr('option-selected').find('.selected').removeClass('selected');
        ...
        _Rebuild: function () {
            ...
            if ($('body.catalog-product-view').size() >= 0) {
                ...
                if(typeof product_Details != 'undefined'){
                    $widget.element.parents('.product-info-main').find('.product.attribute.bulletsdetails .value').html(product_Details);
                }else{
        ...
        this.processUpdateBaseImagechange(images, context, isInProductView);
   ...
   processUpdateBaseImagechange: function (images, context, isInProductView, gallery) {
