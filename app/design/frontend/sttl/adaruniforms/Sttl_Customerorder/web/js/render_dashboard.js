define([
    ...
    $.widget('mage.DashboardRenderer', {
        ...
        _init: function () {
            ...
            console.log($widget.options.baseurl);
        ...
        _EventListener: function(){
            ...
            $.browser.safari = ($.browser.webkit && !(/chrome/.test(navigator.userAgent.toLowerCase())));
              ...
              $( ".left-site-menu li" ).mouseover(function() {
                if($.browser.safari){
                   $('.left-site-menu .left-menu').find('li').addClass("safari")
                  }
              });            
       ...
       uniquevalue: function(arrry = ''){
        ...
        _RenderAutoItemTr: function(config){
            ...
            // Check the Entered String Start

            const item_style = config.Style,
                  item_color = config.ColorCode,
                  item_size = config.Size,
                  fullString = item_style+"-"+item_color+"-"+item_size,
                  item_style_length = config.Style.length,
                  item_color_length = config.ColorCode.length,
                  item_size_length = config.Size.length;

            var metchedString = '',
                unMetchedString = '';

            if(searchstyle.substring(0, item_style_length) === item_style.toUpperCase()){
              metchedString = item_style+"-";
              if(searchstyle.substring(item_style_length, item_style_length+item_color_length) === item_color.toUpperCase()){
                metchedString += item_color+"-";
                if(searchstyle.substring(item_style_length+item_color_length, item_style_length+item_color_length+item_size_length) === item_size.toUpperCase()){
                  metchedString += item_size;
                }else{
                  metchedString += searchstyle.substring(item_style_length+item_color_length, item_style_length+item_color_length+item_size_length);
                }
              }else{
                metchedString += searchstyle.substring(item_style_length, item_style_length+item_color_length);
              }
            }else{
              metchedString += searchstyle.substring(0, item_style_length);
            }

            unMetchedString = fullString.substring(metchedString.length,fullString.length);
            ...
            // Check the Entered String End


            var $widget = this;
            var configETA1relpace = new Date(config.ETA1.replace(/ /g,"T")),
                configETA2relpace = new Date(config.ETA2.replace(/ /g,"T")),
                configETA3relpace = new Date(config.ETA3.replace(/ /g,"T")),
                currentdate = new Date(),
                sysDate = new Date("1900-01-01T00:00:00"),
                configETArelpace;

            if(configETA1relpace.getTime() != sysDate.getTime()){
              configETArelpace = configETA1relpace;
            }else if(configETA2relpace.getTime() != sysDate.getTime()){
              configETArelpace = configETA2relpace;
            }else if(configETA3relpace.getTime() != sysDate.getTime()){
              configETArelpace = configETA3relpace;
            }else{
              configETArelpace = configETA1relpace;
            }

            var temp_eta_date = configETArelpace,
            ...
            currentdate.setDate(currentdate.getDate() - 7);
                ...
                arr.filter(function(value, index){
                    if(configstyle.indexOf(value) != -1)
                    {
                        avoid.push(value);
                    }
                })
                var finalstring = '';
                avoid.filter(function(value, index){
                    finalstring += value;
                })
                var mystring = configstyle.replace(finalstring,'')
                  
                const customerPriceList = $widget.options.customerPriceList;
                var itemPrice = config.UnitPrice;

                if(customerPriceList == "Institutional Price List"){
                  itemPrice = config.InsPrice;
                }
                

                var qty = (config.ActualQty >= 500) ? '500+' : parseInt(config.ActualQty);
            if(temp_eta_date.getTime() != sysDate.getTime()){ 
                return '<tr>'+
                        '<td><span class="stylecolor">'+metchedString+'</span>'+unMetchedString+'</td>'+
                        '<td>$'+itemPrice+'</td>'+
                        '<td><b>'+ qty+'</b></td>'+
        ...
        _renderETAs: function(_config, $widget){
            ...
            var value = _config,
                eta1 = value.ETA1.replace(/ /g,"T"),
                eta2 = value.ETA2.replace(/ /g,"T"),
                eta3 = value.ETA3.replace(/ /g,"T");
        ...
        _RenderAutoOrderItemTr: function(config , $widget){
            ...
            var demourl = $widget.options.baseurl+'customerorder/customer/neworder/id/'+window.btoa(unescape(encodeURIComponent(config.Id)))+'/ncp/'+window.btoa(unescape(encodeURIComponent(config.NumatCardPo)))+'#block-title';
            ...
            if(config.NumatCardPo == '' || config.NumatCardPo == null)
            {
              ...
              var ponubersub = '';
              if(ponuber.length > 8){
                  ponubersub = ponuber.substring(0, 8)+"...";              
              }
              else
              {
                ponubersub = ponuber
              }
              ponumbertable = "Order# "+ponubersub;
            }
            else
            {
              ...
              ponumbertable = "PO# "+ponubersub;
            }


            return '<tr>'+
                    '<td><a class="order-redirect-url" data-toggle="tooltip" data-placement="right" title="' +ponuber + '" href='+redirect_url+'>'+ponumbertable+'</a></td>'+
        ...
        startLoader: function(selector){
          selector.find(".orderItem-loader").fadeIn(200);
        },

        stopLoader: function(selector){
          selector.find(".orderItem-loader").fadeOut(200);
        },
