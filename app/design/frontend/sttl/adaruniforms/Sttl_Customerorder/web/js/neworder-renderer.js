...
], function($, mageTemplate, customerData, thumbnailPreviewTemplate, lineitemstemp, marketinglineitem, marketingitem, shiptracking, inventory, ionRangeSlider) {
    "use strict";
    var activecat = [],
        activecat_material = [],
        activecol = [],
    ...
    $.widget("mage.SwatchRenderer", {
        ...
        _RenderAutoItemDivLi: function(config, i, option) {
            ...
            return (
                ...
                '" >' +
                "<span>" +
                (option == 1 ? config.Style : config.ItemCode) +
                " - " +
                config.ShortDesc +
        ...
        getColorProductArray: function(sku) {
            ...
            if(typeof final_vale_array[rendom] != 'undefined'){
                if(final_vale_array[rendom]['U_WImage1'] == ''){
                        rendom = final_vale_array.length > 0 ? Math.floor(Math.random() * final_vale_array.length + 1) : 0;
                        rendom = rendom >= 0 && rendom < final_vale_array.length ? rendom : 0;
                }
            }
        ...
        _init: function() {
            ...
            $("#po_number").focus();
            if ($("#po_number").is(":focus") && $('#po_number').val().length > 1 && $('.newOrderStep2').css("opacity") != 0.5) {
              $('.editpoorder').show();
            }
        ...
        _create: function() {
            ...
            $("#po_number").focus();
            if (!base_order_id) element.find("#po_number").css({ opacity: "" }).attr("readOnly", false);
            element.find(".checkPoAndInsert").on("click", function(e) {
                ...
                console.log("po",$widget.options.poChec)
            });
            $(document).on("click", ".saveData", function(e, data) {
                ...
                if (!valIsExists && isEdit == 0) {
                    ...
                    $("#message").fadeOut(300);
                    if ($(this).parents(".cf").find(".edit_note").is(":visible")) $(this).parents(".cf").find(".edit_note").fadeOut(300);
                }
                var checklink = $(this).attr("disabled");
                if (valIsExists || isEdit == 1) {
                    var getColorCode = "";
                    if ($(".swatch-option.image.active").length > 0) getColorCode = $(".swatch-option.image.active").attr("option-color-code");
            ...
            if (item_edited) {
                $.ajax({
                    ...
                    success: function(response) {
                        $widget._checkOrderStatus($widget,response);
                        ...
                        if (response.errors) {
                            ...
                            if (response.message) $widget._addOpt1error(response.message);
                            $widget._changeurl(response.base64_order_id, response.base64_ncp_id);
                            if (lineitem_temp) $(".cf.delOrdLink").fadeIn(300);
                            $("#is_edit_order").attr("value", "0");
                        }
                        if (response.success) {
                            if (response.base64_order_id && response.base64_ncp_id) $widget._changeurl(response.base64_order_id, response.base64_ncp_id);
                            $("#is_edit_order").attr("value", "0");
                        }
        ...
        */
        _checkOrderStatus: function($widget,response){
            if (typeof response.orderStatus != 'undefined' && response.orderStatus != 'Draft') {
              const orderId = response.orderId;
              const warningHTML = $widget._renderOrderSubmittedWarning(response.message);
              if(!$("#maincontent").children('.container').find('#orderWarningWrapper').length){
                $("#maincontent").children('.container').append(warningHTML).focus();
              }
              $widget._changeurl();
              setTimeout(function(){
        ...
        _renderOrderSubmittedWarning(message){
          return '<div id="orderWarningWrapper" class="orderWarning-wrapper">'+
                  ...
                  message+
                  '</p>'+
                  '<button class="order-warning-popup-btn" onclick="location.reload()">Ok</button>'+
                  '</div>'+
                  '</div>';
        },
        ...
        _EventListener: function() {
            ...
            $widget.element.on("click", ".num-in span", function(e) {
              ...
              $(document).keyup(function(e){
                if (e.keyCode === 27) {
                     if(!$('.imgquickViewCont').hasClass('_show') && !$('#popupModal').hasClass('show') )
                     {
                ...
                $(document).on('click', '.swtach .swatch-option.image', function() {
                    ...
                    $('.size_table_sec.newOrderStep4 .alignRight a.saveData.themeBtn').css({'display':'block'});
                    ...
                    setTimeout(function(){
                        if($('#'+id+' .colorContainer table.table.table-bordered.table-responsive tbody').width() < $('#'+id+' .colorContainer table.table.table-bordered.table-responsive tbody tr').width()){
                           $('.colorContainer table.table.table-bordered.table-responsive tbody').css('cursor','move')
                        }else{
                            $('.colorContainer table.table.table-bordered.table-responsive tbody').css('cursor','default')
                        }
                    },200)
                    ...
                    function getFocusWithoutScrolling() {
                        ...
                        active_table.getElementsByClassName("qtyTd")[0].firstElementChild.focus({ preventScroll: true });
                    }
             ...
             $(document).on('click', '.collapse_product button', function (e) {
              if ($(".collectionTabs.nav-tabs").is(':visible')) {
                ...
                if($("#browse_collection").hasClass("active")){
                    $("#browse_collection #user-gird-conatiner").slideUp(500, 'linear', function(){
                        $(".collectionTabs.nav-tabs").slideUp(100, 'linear');
                    });
                }else{
                    $("#marketing_materials").slideUp(500, 'linear', function(){
                        $(".collectionTabs.nav-tabs").slideUp(100, 'linear');
                    });
                }
              }else{
                ...
                if($("#browse_collection").hasClass("active")){
                    $(".collectionTabs.nav-tabs").slideDown(100, 'linear', function(){
                        $("#browse_collection #user-gird-conatiner").slideDown(500, 'linear');
                    });
                }else{
                    $(".collectionTabs.nav-tabs").slideDown(100,'linear', function(){
                        $("#marketing_materials").slideDown(500, 'linear', function(){
                            $("#marketing_materials").css("display","");
                        });
                    });
                }
              }
            ...
            $widget.element.on("input", ".dropdown", function(e) {
             ...
             $("#po_number").on("keyup change", function(e) {
                var ponumberText = $(this).val();
                if ($('#po_number').val().length < 3) {
                        $('form#createorder').find(".po-exist").html("");
                }
            });
            ...
            $(document).on('focusout','#po_number', function(e) {
                ...
                if ($('#po_number').val().length > 3) {
                    if(!$('form#createorder .newOrderStep2').css("opacity") != 0.5){
                        $('.editpoorder').show();
                    }
                }
                $('.old_po').attr('value',ponumberText);
                  if($.trim(ponumberText).length > 0)
                  {
                          ...
                          var newpo = $('.editpoorder').attr('po_number');
            ...
            $(document).on('click','i.fa.fa-pencil.editpoorder', function(e) {
                var ponumberval = $("#po_number").val();
                    $('form#createorder').find("#po_number").css({ opacity: "" }).attr("readOnly", false);
                    $("#po_number").attr("disabled", false);
                    $('form#createorder').find("#po_number").focus();
                    $(this).attr('po_number',ponumberval);
                    $(this).hide();
                // $('i.fa.fa-pencil.editpoorder').css('display','none');
            }) 
            $(document).on('click', function(event) {
                if (!$(event.target).closest('div#user-gird-conatiner .product-group .filter-popup, .popup,.price-checkbox span').length) {
            ...
            $(document).on("click", ".product-group .popup #more-filter", function(e) {
                ...
                var condition = $(".gender-filter :checkbox:checked").length
                if(fromprice > 0 || toprice < max){
                    condition = condition + 1;
                }
            ...
            $widget.element.on("click", ".num-in .plus", function(e) {
                ...
                if (!$(this).attr("data-index")) $(this).attr("data-index", 1);
                else $(this).attr("data-index", parseInt($(this).attr("data-index")) + 1);
                ...
                var parantclass = $(this).parent().attr("class"),
                    ...
                    co = 5;
                let itemdata = { item_sku: productid, qty_ordered: qty_ordered_tmp, unitprice: unitprice_tmp };
            ...
            $widget.element.on("click", "span.minus", function(e) {
                ...
                if (!$(this).attr("data-index")) $(this).attr("data-index", 1);
                    ...
                    .parents(".num-in")
                    .find("span.plus")
                    .attr("data-index", parseInt($(this).attr("data-index")) - 1);
            ...
            $(document).on("keydown", "#opt_two_qty", function(e) {
                ...
                if (keyval == 9 || keyval == 13) {
                    $(".add-to-order-main").trigger("click");
                    return false;
                }
            });
            $(document).on("click", ".add-to-order-main", function(e) {
                ...
                if (opt_two_sku != "" && opt_two_qty != "") return $widget._optiontwoAddqty(opt_two_qty, opt_two_sku, $("#po_number").val(), $widget);
            });
            ...
            $(window).resize(function() {
                var colorid;
                if ($(".swatch-option.image.active").length > 0) colorid = $(".swatch-option.image.active").attr("option-color-code");
                if(colorid){
                    if($('#'+colorid+' .colorContainer table.table.table-bordered.table-responsive tbody').width() < $('#'+colorid+' .colorContainer table.table.table-bordered.table-responsive tbody tr').width()){
                       $('.colorContainer table.table.table-bordered.table-responsive tbody').css('cursor','move')
                       console.log("move_color")
                    }else{
                        $('.colorContainer table.table.table-bordered.table-responsive tbody').css('cursor','default')
                       console.log("not_move_color")
                    }
                }else{
                    if($('.colorContainer table.table.table-bordered.table-responsive tbody').width() < $('.colorContainer table.table.table-bordered.table-responsive tbody tr').width()){
                       $('.colorContainer table.table.table-bordered.table-responsive tbody').css('cursor','move')
                       console.log("move")
                    }else{
                        $('.colorContainer table.table.table-bordered.table-responsive tbody').css('cursor','default')
                       console.log("not_move")
                    }
                }
            ...
            $widget.element.on("click", ".catBtns .customBtns", function() {
                ...
                if (active_pro == "config_" + cur_button) return false;
                else {
            ...
            $(document).on("mouseover", ".product_options.createOrder", function(e) {
                ...
                imgs.each(function(index, value) {
                    var img = $("<img />", { class: "popupImage", src: $(this).attr("data-href"), alt: $(this).attr("alt") });
        ...
        _updateponumberedit: function($widget){
              ...
              var falg = _.filter(this.options.poCheck, function(value) {
                return value["NumatCardPo"] === newpo;
              });
              var editpodashboard = $('.editpoorder').attr('po_number');
              if (falg.length > 0 && editpodashboard != $('#po_number').val()) {
                $('form#createorder').find(".po-exist").addClass("message-error error").html("PO Number already exists.");
                return false;
              }else{
                $('form#createorder').find(".po-exist").addClass("message-error error").html("");
                $('form#createorder').find(".po-exist").removeClass("message-error error");
                if(newpo.length >= 4)
                  {
                       jQuery.ajax({
                          url: url,
                          ...
                          success: function(response){
                              if(response)
                              {
                                  var responce = response,
                                      ...
                                      poconfig_data = $widget.options.poCheck,
                                  ...
                                  if(falg.length > 0){
                                    ...
                                    if (response.base64_order_id && response.base64_ncp_id) {
                                        var form_action = $widget.options.baseurl+"customerorder/customer/payment?back_order_id="+response.base64_order_id+"&back_po_number="+response.base64_ncp_id;
                                        $("form#createorder").attr("action",form_action)
                                        $widget._changeurl(response.base64_order_id, response.base64_ncp_id);
                                    }
                                  }
                              }
        ...
        _determineProductData: function(PoVal) {
            console.log("this.options.poCheck",this.options.poCheck)
            var falg = _.filter(this.options.poCheck, function(value) {
                return value["NumatCardPo"].toLowerCase() === PoVal.toLowerCase();
            });
            console.log("flag",falg)
            var editpodashboard = $('.editpoorder').attr('po_number');
            if (falg.length > 0 && editpodashboard != PoVal) {
        ...
        _getLineItemTable: function(order_id) {
            ...
            $.ajax({
                url: req_url,
                type: "POST",
                data: { base_order_id: order_id, po_number: tmp_po_number, get_order_data: 1 },
                showLoader: false,
                cache: false,
                success: function(response) {
                    $widget._checkOrderStatus($widget,response);
        ...
        _OnChangeOptTwoInfo: function($this, $widget, keyup = 0) {
            ...
            if ((keyup = 1)) {
                result = this.getProductArray($("#opt_tow .element.autocomplete-active").find("input").val(), 2);
            }
            if (result.length > 0) {
        ...
        _OnChangeDropdown: function($this, $widget) {
            ...
            var a,
                ...
                input = "",
                res = [];
            var arr1 = $this.attr("data-option") == 1 ? this.options.ConfigStyle : this.options.jsonConfig;
            var arr =  sortResults('Style', true)
            ...
            if ($this.attr("data-option") == 2) {
                setTimeout(function(e) {
                    $widget._OnChangeOptTwoInfo($this, $widget, 1);
                }, 200);
            }
        },
                ...
                if (option == 1) {
                    ...
                    var first_colorcode = '';
                     if ($(".product_options").find(".core-color-section").length > 0) {
                        first_colorcode = $(".customorder-color.core-color-section .swatch-option.image").first().attr('option-color-code');
                    } else {
                        first_colorcode = $(".customorder-color.fashion-color-section .swatch-option.image").first().attr("option-color-code");
                    }
                    // console.log(active_exist);
                    var styleconfiguration = mageTemplate(thumbnailPreviewTemplate, {
                        ...
                        first_colorcode: first_colorcode,
                        mapprice: mapprice,
                        hideprice: hideprice
                    });
                    ...
                    $(".product-slider")
                        .find("#" + temp_parent_sku + "")
                        .addClass("pro_active");
                    var active = temp_parent_sku;
                    jQuery('.product-slider .owl-stage div.product[id="' + active + '"]')
                        .addClass("pro_active")
                        .parent()
                        .addClass("sticky");
                    ...
                    if ($(".owl-item.sticky.active").length > 0) $(".product-slider-sticky").remove();
                    if (data !== undefined) {
        ...
        _OnEnterClick: function($this, $widget, e) {
            if (e.which == 9 || e.which == 13) {
                ...
                if ($(".autocomplete-items").length > 0) {
                    $this.val(
                        $("#" + $this.attr("id") + "autocomplete-list")
                        .find(".element.autocomplete-active input")
                        .val()
                    );
                }
                if ($this.attr("data-option") == 1) $(".searchFromStyle").trigger("click");
                else if ($this.attr("data-option") == 2) {
                    if (!$widget._checkSkuAvailable($this.val())) {
                        return false;
                    }
                    $("#opt_two_qty").focus();
                    this._OnChangeOptTwoInfo($this, $widget);
                } else if ($this.attr("data-option") == 3) $("#" + $this.attr("id") + "autocomplete-list").remove();
                return false;
                ...
                if (active >= scroller && e.which == 40) {
                    ...
                    list.find("div").each(function() {
                        ...
                        if (index1 >= scroller && index1 <= index) totalscroller += $(this).innerHeight();
                    });
                } else if (e.which == 38) {
                    ...
                    list.find("div").each(function() {
                        var index1 = $(this).attr("data-index");
                        if (index1 < index && index1 <= last) totalscroller += $(this).innerHeight();
                    });
                }
        ...
        _collectionlogoslider: function() {
            ...
            $.each(logos, function(key, logos) {
                if (logos.name != "Other") {
                    ...
                    html += logos.image ? "<img src=" + logos.image + " alt=" + logos.name + ">" : "<h1 class='collection-logo'>" + logos.name + "</h1>";
                    html += "</li>";
                }
            });
            var coll = $(".Collections").owlCarousel({ loop: false, autoplay: false, autoplayTimeout: 30000, margin: 10, nav: false, dots: false, responsive: { 0: { items: 2 }, 600: { items: 6 }, 1000: { items: 6 } } });
        ...
        productslider: function() {
            ...
            owlproduct = $(".product-slider").owlCarousel({
                ...
                responsive: { 0: { items: 1 }, 600: { items: 2 }, 800: { items: 4 }, 1000: { items: 5 }, 1100: { items: 5 }, 1200: { items: 6 }, 1300: { items: 7 } },
        ...
        categoryclick: function($this) {
            ...
            if (vid == "All" || (activecat.length == 0 && vid == null)) {
                ...
                if (value.length == 0) {
                    var html = "<div class='item product product-item not-item'>Item not Found</div>"
        ...
        resetslidervalue: function(prohtml, items) {
            if (prohtml) {
                ...
                if(items == 0){
                    ...
                    $(".product-slider").find(".owl-item .product-item").removeClass("pro_active");
                    owlproduct.trigger("replace.owl.carousel", prohtml, { options: { loop: false, mouseDrag: false } });
                    $(".product-slider  .owl-controls").css("display","none");
                    owlproduct.trigger("refresh.owl.carousel");
                } else if (items > 0 && items < 7) {
                    owlproduct.trigger("replace.owl.carousel", prohtml, { options: { loop: false, mouseDrag: false } });
                    $(".product-slider  .owl-controls").css("display","none");
                    owlproduct.trigger("refresh.owl.carousel");
                    $(".product-slider .owl-stage").addClass("jcenter");
                } else {
                    owlproduct.trigger("replace.owl.carousel", prohtml, { options: { mouseDrag: true, loop: true } });
                    owlproduct.trigger("refresh.owl.carousel");
        ...
        slidervalue: function(res) {
            res = res.filter((thing, index, self) => index === self.findIndex((t) => t.Style === thing.Style));
            var sliderhtml = "";
            var obj = {};
            var $widget = this;
            ...
            $.each(res, function(key, val) {
                ...
                sliderhtml +=
                    "<div class='item product product-item' gname=" +
                    val.GroupName +
                    " id=" +
                    val.Style +
                    ">  <a class='product-item-link'> <span class='product-image-wrapper' style='padding-bottom: 133.94495412844%; width: auto;'> <img class='img-responsive owl-lazys' src='" +
                    (val.U_WImage1 ? val.U_WImage1 : placeholder) +
                    "' width='218' height='292' alt=" +
                    ItemName +
                    " /></span> </a> <div class='product details product-item-details'> <strong class='product name product-item-name'> <a class='product-item-link' title=" +
                    ItemName +
                    " > " +
                    ItemName +
                    " </a> </strong> <div class='show-product-dis-box'><span>" +
                    parentid +
                    "</span>" +
                    dtag +
                    "</div><div class='show-product-dis-box-more'> <span> <lable>Style No.</lable> </span> <span>" +
                    val.Style +
                    "</span> </div>";
        ...
        _conditioninfoDescripion: function($widget, itemcode) {
            ...
            if (item.is_active == 2) {
                ...
                if (item.price == 0) {
                    ...
                    message +=
                        "<div class='marketing-item-notes-tooltip'> <div><span>Minimum total order of $" +
                        $widget._convertcurrency(parseInt(item.minimum_order_amt)) +
                        " required to purchase.</span></div>" +
                        "<div> <span>1 FREE pack with every purchase of $" +
                        $widget._convertcurrency(parseInt(item.free_after)) +
                        "</span></div> </div>";
                }
            }
            return message;
        ...
        _checkcondition: function($widget, update_slider = false, click_event = "") {
            ...
            $.each(mark_invdata, function(index, value) {
                ...
                $(".mark-slider .owl-item")
                    .find("#" + itemcode + " .num-in .qty_add_num")
                    .val(currentitem_qty);
                if (currentitem_qty > 0) {
                    $(".mark-slider .owl-item")
                        .find("#" + itemcode + " .num-in .minus")
                        .removeClass("disable");
                }
                var qty = $(".mark-slider .owl-item")
                    .find("#" + itemcode + " .num-in .qty_add_num")
                    .attr("value");
                ...
                if (TotalBeforeDiscount >= minimum_price) {
                    ...
                    if (TotalBeforeDiscount >= parseFloat(free_after)) {
                        if (new_freeafter_total <= TotalBeforeDiscount) {
                            ...
                            $(".mark-slider .owl-item")
                                .find("#" + itemcode + " .description .product-item-price")
                                .html("$0.00");
                            $(".mark-slider .owl-item")
                                .find("#" + itemcode + " .num-in .qty_add_num")
                                .attr("is_free", "true");
        ...
        _renderLineitembeforeAJAX: function($this, _current_options, styles = {}, click_event = "") {
            ...
            if (result.length > 0) {
                var marketing_lineitem = mageTemplate(marketinglineitem, { marketingitems: result, current_item: $.proxy(this._getMarkItemProductInfo, this) });
            ...
            finalitems.forEach(function(item, index) {
                ...
                config_total_data[item.Style] = { total_qty: total_qty_tmp, total_price: total_price_tmp };
            });
        },
        ...
        _generateDiscountTooltip: function($widget) {
            ...
            if (customrsbulkdiscount.length > 0) {
                ...
                customrsbulkdiscount.forEach(function(item, index) {
                    if(parseInt(DocTotalQty) >= parseInt(item.QtyFrom) && parseInt(DocTotalQty) <= parseInt(item.QtyTo)){
                        customrsbulkdiscount.forEach(function(itema, indexa) {
                            if(parseInt(item.QtyTo)+2 > parseInt(itema.QtyFrom) && parseInt(item.QtyTo) < parseInt(itema.QtyTo)){
        ...
        _generateqtyarray: function(_current_options, $widget, styles = {}) {
            ...
            if (current_options == 1 && current_options != 0 && current_options != "") {
                $(data_selector).each(function() {
                    ...
                    if ($(this).val() != "") {
                        ...
                        if (parseFloat(disprice) < parseFloat(base_price)) {
            ...
            if (current_options == 5 && current_options != "") {
                var itemvalue = styles,
                    ...
                    is_item_free = $(".mark-slider .owl-item")
                    .find("#" + current_itemcode + " .num-in .qty_add_num")
                    .attr("is_free"),
                            ...
                            for (var i = 1; i < rows.length - 1; i++) {
                                ...
                                for (var m = 1; m < cells.length; m++) {
                                    ...
                                    if (cellval == '') {
                                        ...
                                        if ($widget._isNormalInteger(cellval) && is_valid) {
                                            tmp_qty += cellval;
                                        } else {
                                            is_valid = false;
                                            tmp_qty = '';
                                ...
                                tmp_qty = tmp_qty.trimStart();
                                tmp_qty = tmp_qty.trimEnd();
                                if (/\s/.test(tmp_qty)) {
                                    tmp_qty = '';
                                }
                ...
                if (opt_two_sku != "" && regexAll.test(opt_two_sku)) {
                    if ($widget._isNormalInteger(opt_two_qty) && opt_two_qty != "") {
                        if ($widget._checkSkuAvailable(opt_two_sku)) {
                            ...
                            if (disprice < base_price) pafterdiscount = added_qty * disprice;
                            else pafterdiscount = added_qty * base_price;
                            var tmpitem = {},
                            ...
                            if (backorder_qty < 0) {
                                ...
                                if (eta_date > currentdate) {
                                    backorder_date = eta_date.getMonth() + 1 + "/" + eta_date.getDate() + "/" + eta_date.getFullYear().toString().substr(-2);
                                    var tmp_arra = { sku: opt_two_sku, linenumber: linenumber, backorderqty: backorder_qty, backordereta: backorder_date };
                            ...
                            var tmp_arra = { sku: opt_two_sku, linenumber: linenumber };
                            itemnotavailablearray.push(tmp_arra);
                        }
                        linenumber++;
                    } else {
                        if (opt_two_qty === "" || regexAll.test(opt_two_qty)) {
                            var tmp_arra = { sku: opt_two_sku, linenumber: linenumber, message: "Qty is not valid" };
                    ...
                    if ((opt_two_sku === "" && opt_two_qty != "") || regexAll.test(opt_two_sku)) {
                        var tmp_arra = { sku: opt_two_sku, linenumber: linenumber, message: "SKU is not valid" };
                        qtynotvalid.push(tmp_arra);
                    }
                    linenumber++;
                }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.errors) {
                        ...
                        if (response.base64_order_id) $("#base64_order_id").val(response.base64_order_id);
                        if (response.base64_ncp_id) $("#base64_ncp_id").val(response.base64_ncp_id);
                        if (response.message) $widget._adderror(response.message);
                        $widget._changeurl(response.base64_order_id, response.base64_ncp_id);
                        if (lineitem_temp) $(".cf.delOrdLink").fadeIn(300);
                    }
                    if (response.success) {
                        ...
                        if (response.base64_order_id) $("#base64_order_id").val(response.base64_order_id);
                        if (response.base64_ncp_id) $("#base64_ncp_id").val(response.base64_ncp_id);
                        if (response.message) $widget._addsuccess(response.message);
                        $widget._changeurl(response.base64_order_id, response.base64_ncp_id);
                        $(".colorContainer .checkvalue, .colorContainer .unittotal").val("");
                        $(".colorContainer .showprice, .maxqtyvaldi").html("");
                    }
        ...
        mark_material_slidervalue: function(res) {
            ...
            $.each(res, function(key, val) {
                ...
                var mark_item = mageTemplate(marketingitem, { data: val, baseurl: baseurl, description: $widget._conditioninfoDescripion($widget, current_item), currencyconvert: $.proxy($widget._convertcurrency, this) });
        ...
        mark_material_resetslidervalue: function(materialhtml, items) {
            ...
            if (materialhtml) {
                ...
                if (items <= carousel.owlCarousel.settings.items) {
                    owlmaterial.trigger("replace.owl.carousel", materialhtml, { options: { loop: false, mouseDrag: false } });
                    owlmaterial.trigger("refresh.owl.carousel");
                    $(".owl-controls").css("display", "none");
                } else {
                    owlmaterial.trigger("replace.owl.carousel", materialhtml, { options: { mouseDrag: true, loop: true } });
                    owlmaterial.trigger("refresh.owl.carousel");
