            ...
            _EventListener: function () {
                ...
                $(document).on('click','#searchfindorder',function(e){                    
                    event.preventDefault();
                        var current_var = $("input#order_po_serach");
                        var len = $('#order_po_serach').val().trim().length;
                        var first = $('#order_po_serach').val().substring(0,1);
                        if(len < 5 || first == ' ')
                        {
                        ...
                        if(len >= 5 && first != ' '){
                        var moreval = $('#order_po_serach').val().toLowerCase();
                            script.showLoader();
                            setTimeout(function(){
                ...
                $(document).on('input','#order_po_serach',function(e){                    
                    var order_po = $('#order_po_serach').val();
                    // var keyCode = e.keyCode || e.which;
                    script.searchOrder(options.BaseUrl, options.orderlist, $(this), totalorders);
                   
                });

                $(document).on('keypress','#order_po_serach',function(e){                    
                    var order_po = $('#order_po_serach').val();
                    var keyCode = e.keyCode || e.which;
                    if(keyCode == 13){
