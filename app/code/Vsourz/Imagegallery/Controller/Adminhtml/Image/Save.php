...
class Save extends \Vsourz\Imagegallery\Controller\Adminhtml\Image
{
    ...
    public function execute()
    {
        ...
        if (isset($formPostValues['imagedata'])) {
			...
			$imageData['image_type'] = ($imageData['image_type']  == "Image") ? 0 : 1;
            $imageId = isset($imageData['image_id']) ? $imageData['image_id'] : null;
			if ($imageData['image_type'] == 0) { 
				unset($imageData['image_iframe']);
			}
			
			  if (isset($image) && isset($image['name']) && $image['name'] != null && $imageData['image_type'] == 0) {
				...
				if ($imageData['image_type'] == 1 && isset($imageData['image_iframe'])) {
					$imageData['image_photo'] = $imageData['image_iframe'];
				}
            }
