        ...
        if ($data = $this->getRequest()->getPostValue()) {
            ...
            if ($MobileimageRequest) {
                if (isset($MobileimageRequest['name'])) {
                    $mobilefileName = $MobileimageRequest['name'];
                } else {
                    $mobilefileName = '';
                }
            } else {
            ...
            if($MobileimageRequest && strlen($mobilefileName))
            {
                try {
                $mobileuploader = $this->_uploaderFactory->create(['fileId' => 'mobile_image']);
                    $mobileuploader->setAllowedExtensions(['jpg', 'jpeg', 'gif', 'png']);
                    $mobileimageAdapter = $this->_adapterFactory->create();

                    $mobileuploader->addValidateCallback('banner_image', $mobileimageAdapter, 'validateUploadFile');
                    $mobileuploader->setAllowRenameFiles(true);
                    $mobileuploader->setFilesDispersion(true);

                    $mediaDirectory = $this->_objectManager->get('Magento\Framework\Filesystem')
                        ->getDirectoryRead(DirectoryList::MEDIA);

                    $mobileresult = $mobileuploader->save(
                        $mediaDirectory->getAbsolutePath(\Magestore\Bannerslider\Model\Banner::BASE_MEDIA_PATH)
                    );
                    $data['mobile_image'] = \Magestore\Bannerslider\Model\Banner::BASE_MEDIA_PATH.$mobileresult['file'];
...
//            $localeDate = $this->_objectManager->get('Magento\Framework\Stdlib\DateTime\TimezoneInterface');
            ...
            $data['video_iframe'] = !empty($post_data['video_iframe']) ? $post_data['video_iframe'] : '';
