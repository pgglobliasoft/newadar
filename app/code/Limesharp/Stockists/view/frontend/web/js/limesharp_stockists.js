		...
		return function (config) {
	        ...
	        $(document).ready(function() {
	            ...
	            $("#stockists-submit").on("click", function(e) {
	                setTimeout(function() {
	                		$(".stockists-results li:first").click();
	                	}, 100);
	            ...
	            function getStores() {
	                var url = window.location.protocol+"//"+window.location.hostname+((window.location.port != undefined) ? ':'+window.location.port : '') +window.location.pathname;
	            ...
	            function initialize(response) {
	                ...
	                function bindInfoWindow(marker, map, infowindow, name, address, city, postcode, telephone, link, external_link, email,country,region) {
	                    google.maps.event.addListener(marker, 'click', function() {
	                        var contentString = '<div class="stockists-window" data-latitude="'+marker.getPosition().lat()+'" data-longitude="'+marker.getPosition().lng()+'"><h3>'+name+'</h3>'
	                        if (address) {
	                        ...
	                        if (city) {
	                            contentString += '<p class="stockists-telephone">'+city+ ' '+getregion+ ', ' + postcode+'</p>'
	                        }
	                        /*if (postcode) {
	                            contentString += '<p class="stockists-web">'+postcode+'</p>';
	                        }*/
	                        contentString += '<p class="ask-for-directions get-directions" data-directions="DRIVING">Get Directions<span class="fa fa-caret-right"></span></p>';
	                        contentString += '</div>';
	                ...
	                for (var i = 0; i < length; i++) {
	                    ...
	                    bindInfoWindow(marker, map, infowindow, data.name, data.address, data.city, data.postcode, data.phone, data.link, data.external_link, data.email, data.country, data.region);
