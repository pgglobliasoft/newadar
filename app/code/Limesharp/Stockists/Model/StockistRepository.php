...
class StockistRepository implements StockistRepositoryInterface
{
    ...
    public function importDatalocation($data)
    {
      $importData = [];
      $storeId = $this->storeManager->getStore()->getId();
      foreach($data as $locationdata)
      {
        $stockist = '';
          try {
                $stockist = $this->stockistInterfaceFactory->create();
                $stockist->setStoreId($storeId);
                $stockist->setName($locationdata['LocationName']);
                $stockist->setAddress($locationdata['Address']);
                $stockist->setCity($locationdata['City']);
                $stockist->setCountry($locationdata['Country']);
                $stockist->setPostcode($locationdata['Zip']);
                $stockist->setLatitude($locationdata['Latitude']);
                $stockist->setLongitude($locationdata['Longitude']);
                $stockist->setStatus('1');
                $stockist->setPhone($locationdata['Phone']);
                $this->resource->save($stockist);
              } catch (\Exception $exception) {
              throw new CouldNotSaveException(__(
                  'Could not save the store: %1',
