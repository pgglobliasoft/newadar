...
class Rule extends AbstractModel implements IdentityInterface
{
    ...
    public function getheadroductId(){

       
        $products = [];$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $temp_array = [];

        $collection = $this->productCollectionFactory->create();
        $productCollection = $collection->addAttributeToSelect('*')
          ->addFilter('type_id', 'configurable');
        foreach ($productCollection as $websiteId => $productDaat) {
            $_children = $productDaat->getTypeInstance()->getUsedProducts($productDaat);
           
            foreach ($_children as $child){
                // $headtherprodut = $this->productCollectionFactory->create()->load($child->getId());              
                $headtherprodut = $objectManager->create('Magento\Catalog\Model\Product')->load($child->getId());
                if(@$headtherprodut['heather_colors']  && $headtherprodut['heather_colors'] !== '' && $headtherprodut['heather_colors'] != 26291){
                    $temp_array[$productDaat->getId()] = $productDaat->getSku();
                     // echo '<br>'.$productDaat->getId();
                    break;  
                }else{
