...

{
	   ...
	   foreach($productArray as $productitem)
       {
			...
			if(trim($productitem['Parent_Style']) != '')
			{
				...
				try 
				{
					...
					$LastupdateQuery = 'update "dbo".ItemAIO set LastUpdated = \''.$date.'\'  where id = ' .$productitem['ID'];
    ...
    public function ImportConfigProduct($productArray,$adar_magento_obj, $fp_log)
	{
		...
		if(!empty($MagentoUpdateStatus))
		{
			$data = $adar_magento_obj->execute($MagentoUpdateStatus);
