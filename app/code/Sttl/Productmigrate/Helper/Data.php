...
class Data extends AbstractHelper {
				...
				if (isset($productitem['Short_Description']) && trim($productitem['Short_Description']) != '') {
					...
					if ($getProductId && $getProductId != '') {
						...
						if (strtolower(trim($productitem['Color_Type'])) == 'seasonal') {
							// $prosduct->setHeatherColors(26341);
					...
					if (strtolower(trim($productitem['Color_Type'])) == 'seasonal') {
						// $product->setHeatherColors(26341);
						$product->setSeasonalcolors($getSeasonalColorsValue['value']);
					}
					// heather color condition
					elseif (strtolower(trim($productitem['Color_Type'])) == 'heather') {
					...
					if(@$productitem['U_Fabric_Care']){
						$product->setUfabriccareurl($this->imgurlupdate($productitem['U_Fabric_Care']));
					}
	...
	public function ImportConfigProduct($productitem, $adar_magento_obj, $fp_log) {
		...
		// 		->addAttributeToFilter('heather_colors', array('nin' => array(231,'' ,26291)))
		// 		->addAttributeToFilter('parent_style', $productitem['Parent_Style'])
		...
		if (@$productitem['U_Fabric_Care'] != '') {
			$Configrebleproduct->setUfabriccareurl($this->imgurlupdate($productitem['U_Fabric_Care']));
		} else {
			fwrite($fp_log, 'Fabric & Care image not exist for ' . $productitem['Parent_Style'] . "\n");
			$this->logger->info('This parent Style not exsting fabric & Care image' . $productitem['Parent_Style']);
		}
		...
		try
		{
			...
			// echo $productitem['RecomProduct'];die;
	...
	public function simpleproductsettoconfigreble($id, $parent_style, $superAttirbuteids, $fp_log, $con_prd_update = false) {
		...
		try {
			...
			foreach ($collection as $simple_item) {
				...
				$associatedProductskus[] = $simple_item->getSku();
