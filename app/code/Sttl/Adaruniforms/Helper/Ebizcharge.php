...
class Ebizcharge extends AbstractHelper
{
    ...
    )
    {
        ...
        $mode = $this->helper->getConfigData("Adaruniforms/ebiz_charge_configuration/ebizmode");  
		if($mode == 1)
		{
	...
	public function getToken()
	{
		...
		$sourcekey = $this->helper->getConfigData("Adaruniforms/ebiz_charge_configuration/sourcekey");
		...
		$pin = $this->helper->getConfigData("Adaruniforms/ebiz_charge_configuration/ebizpin");
		// $pin = 'wSOwCvU'; // SandBox
		//$pin = 'kCOhMZx'; // Live
   ...
   public function getCustomerById($id = 0)
   {
	   try
		{
			// $data = $this->eBizConnect->getCustomer(self::getToken(), $id);
