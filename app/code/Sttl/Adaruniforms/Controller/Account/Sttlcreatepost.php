...
{   
    public function execute()
    {
        ...
        $customer_number = trim($this->getRequest()->getParam('customer_number')); 
        $webaccess_code = trim($this->getRequest()->getParam('webaccess_code')); 
        $incorrectDetails = "";
        $resultRedirect = $this->resultRedirectFactory->create();
        if(isset($customer_number) && $customer_number != '' && isset($webaccess_code) &&  $webaccess_code != '')
        {
            $incorrectDetails = 0;
            ...
            if(count($webCustomerData) > 0){
                $incorrectDetails = 1;
                $customMessage = __('You are not authorised to register Customer Code already exists, please contact us at 718-935-1197 ext. 3.');
                ...
                if(count($helperData) == 0 || !isset($helperData[0])){
                    $incorrectDetails = 1;
                    $customMessage = __('You are not authorised to register, please contact us at 718-935-1197 ext. 3.');
                    $this->messageManager->addError($customMessage);
                }
            }
        }else{
            $customMessage = __('Customer Code and Web Access Code Required Fields.');
            $this->messageManager->addError($customMessage);
