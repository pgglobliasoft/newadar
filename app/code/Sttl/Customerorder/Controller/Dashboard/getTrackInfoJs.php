...
class getTrackInfoJs extends \Magento\Framework\App\Action\Action
{
...
public function execute()
{
    ...
    $logincardcode = "122010";

    $orderLimitData = json_decode($this->getOrderLimitData(),true);


    $order_imit = 0;

    // foreach ($orderLimitData as $key => $value) {
    //     if($value['cardcode'] == $logincardcode){
...
{   
    return $this->configData->getConfigData("Adaruniforms/recentorder_range/ranges");
}
...
public function getOrderes($CardCode) {
    ...
    $orders = $this->sapHelper->getRecentOrderData($CardCode, $po_number = '', $status = '', $q = '',$opt='',$limit=$order_imit);
...
public function getInvoice($all_invoice = "") {
        ...
        if($all_invoice){
            $invoice = $this->sapHelper->getInvoicesData($customer_number,"Open","","","","","DueDate","ASC");
            $invoice_paid = $this->sapHelper->getInvoicesData($customer_number,"Paid","","","","","CreateDate","DESC");
