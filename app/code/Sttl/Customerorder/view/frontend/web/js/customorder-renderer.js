    ...
    $.widget('mage.SwatchRenderer', {
        ...
        */
        ...
        _init: function () {                
            ...
            mark_invdata = $widget.getConfigurableProduct(this.options.jsonConfig , 'Style');
            mark_invdata_ral = mark_invdata;
            if(this.options.base_order_id && this.options.base_order_id != ''){this._getLineItemTable(this.options.base_order_id); $widget._conditiocheck($widget);}
              ...
              var qty = parseInt($(this).parents('.num-in').find('.qty_add_num').val());
                if(!$(this).attr('data-index'))
                  ...
                  var parantclass= $(this).parent().attr('class'),
                      ...
                      style= $widget._getMSimpleProductInfo(productid),
                      co = 5;
                      // console.log(style);
                  $widget._renderLineitembeforeAJAX($widget,co, style);  
            ...
            $widget.element.on('click','span.minus', function(e){
                 ...
                 var qty = $(this).parents('.num-in').find('.qty_add_num').val();
                  ...
                  var  parantclass= $(this).parent().attr('class'),
                    productid = $(this).parents().closest('.product-item').attr('id'),
                    configproduct= $('#'+productid+' .num-in input[name="mark_item_sku"]').val(),
                    style= $widget._getSimpleProductInfo(configproduct),
                    co = 5;
                    $widget._renderLineitembeforeAJAX($widget,co, style);
                if(qty > 0)
                {
        ...
        _changeemptyurl: function(){
            ...
            if (typeof (history.pushState) != "undefined") {
                var nexturl = this.options.baseurl+'customerorder/customer/customorder',
       ...
       OptionChangeUrl:function(base64_order_id,base64_ncp_id)
        {
            if (typeof (history.pushState) != "undefined") {
                var nexturl = this.options.baseurl+'customerorder/customer/customorder/id/'+base64_order_id+'/ncp/'+base64_ncp_id,
                obj = { Title: 'orderpage', Url: nexturl };
                history.pushState(obj, obj.Title, obj.Url);
            }  
...
// <---------------------------------------- AB collection slider [Done] ----------------------------------------------------------->
        ...
        */
        ...
        _conditioninfoDescripion:function(){
            var info = '';
            $.each(this.options.conditionjson , function(index,value){
                info += value.description ? value.description+'<br>' : '';            
            })
            info += '';
            return info;

        },
        _conditiocheck: function($widget){
             var condition = this.options.conditionjson, slider_update = false;
              $.each(condition , function(index,value){
                   var free_qty=value.free_qty;
                  var object = JSON.parse(value.conditions_serialized); 
                  var product  =jQuery.grep(object.conditions, function( n, i ) {if(n.attribute === 'product_sap') return n.value });
                  var count=false;
                  if(object.conditions.length > 0){
                    $.each(object.conditions, function(key,condition){
                      var attribute= condition.attribute;
                    
                      // console.log(attribute);
                      if(attribute=="DocTotal" || attribute == "TotalQtyOrdered")
                      {
                        var TotalBeforeDiscount = ordertotaldata.TotalBeforeDiscount;
                              
                        var valid = eval(ordertotaldata[attribute] + condition.operator + condition.value||0);
                        if(valid)
                          count=true;                       
                        else
                          return false;                                         
                        ...
                        var falge = false;                 
                        $.each(product[0].value , function(index,value){
                          if(config_total_data[value]){                                        
                            var validQue = eval(config_total_data[value].total_qty  + condition.operator + condition.value);                         
                            if(validQue)
                            {                          
                              falge=true;
                            }else{
                    ...
                    if(count){                    
                      ...
                      $.each(mark_invdata, function(index,value){
                         if(value.GroupName == "Pants"){
                            mark_invdata[index].DisPrice = "0.00";
                            mark_invdata[index].InsPrice = "0.00";
                            mark_invdata[index].UnitPrice = "0.00";
                            mark_invdata[index].Size = free_qty;
                         }
                      })
                      $('.mark-slider').find('.owl-item').each(function(){
                          var category = $(this).find('.item.product.product-item').attr('gname');
                          if(category === "Pants"){ // set category dynamically 
                            var price = $(this).find('.num-in input[name="mark_item_price"]');
                            $(this).find('.flex-markitem-align span.product.price.product-item-price').html('$0.00');
                            if(!price.attr('old-price'))
                            price.attr('old-price',price.val());
                            price.val('0.00')
                            $(this).find('.plus').attr('max-plus',value.free_qty);
                          }
                      })
              ...
              if(!slider_update){                
                  mark_invdata = mark_invdata_ral; 
                  $('.mark-slider').find('.owl-item').each(function(){
                        var category = $(this).find('.item.product.product-item').attr('gname');
                        if(category === "Pants"){ // set category dynamically 
                          var price = $(this).find('.num-in input[name="mark_item_price"]');
                          if(price.attr('old-price')){
                            $(this).find('.flex-markitem-align span.product.price.product-item-price').html('$'+price.attr('old-price'));                          
                            price.val(price.attr('old-price'));                          
                          }
                        }
                    })
              }   
        ...
        _renderLineitembeforeAJAX: function($this, _current_options, styles={}) {
        ...
        */
        ...
        _generateqtyarray: function(_current_options, $widget, styles={}) {
          ...
          if(current_options == 5){
            
           
              var opt_two_sku = styles.ItemCode;
             var opt_two_qty = $('#'+styles.Style+' .num-in input.qty_add_num').val();
              var current_pro = $widget._getSimpleProductInfo(opt_two_sku);
                var base_price = current_pro.UnitPrice;
              var disprice = current_pro.DisPrice;
              var added_qty = opt_two_qty;
...
// <-------------------------------------------------------- Marketing Material START BS ------------------------------------------------->
        ...
        _mark_activeclassvalue:function(data={}){
           ...
           var gruopname = this._mark_slidercollection(data),
              result = new Array();
              var array = activecat.map(function (el) {
                  return el.replace("_"," ");
                });
              activecat = this.uniquevalue(array);
             var dd = gruopname.filter(item => activecat.includes(item.category));
             console.log("DD",dd)
             return dd;
        },
        ...
        mark_material_categoryclick: function($this){
          var $widget = this
            ...
            if(vid == "All" || activecat == ''){
                var marketData = this.options.marketData;
                var value = $widget._mark_allcategoryfilter(mark_invdata);
                var datavalue = $widget.mark_material_slidervalue(marketData);
                $widget.mark_material_resetslidervalue(datavalue["html"],datavalue["items"])
            }else{
                var marketData = this.options.marketData; 
                var value = $widget._mark_activeclassvalue(marketData);
