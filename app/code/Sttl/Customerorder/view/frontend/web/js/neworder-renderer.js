...
define([    
    ...
    'text!Sttl_Customerorder/template/Inventory.json',
    'inventory.json'
], function ($, mageTemplate, customerData, thumbnailPreviewTemplate,lineitemstemp, json , opop) {
    'use strict';
     var activecat = [];
     var selector = '';
     ...
     console.log('mobile thingbfhdgf');
    /**
            ...
            $.each(gruname,function(key,val){
                ...
                var proids2 = [];
                    $.each(data,function(prokey,proval){
                        if((proval.Collection == key || proval.Collection == "Other")  && proval.GroupName == val[i] && val[i] != ''){
                            var obj = {};
        ...
        */ 
        ...
        categoryclick: function($this){
            ...
            if($(".Collections .owl-item").hasClass("active_collection")){
                    var value = $widget.activeclassvalue($(".owl-item.active_collection.active"),vid);
                    var datavalue = $widget.slidervalue(value)
                    prohtml = datavalue["html"]
                    items = datavalue["items"]
                }else{
                    var value = $widget.activeclassvalue($(".Collections .owl-item"),vid);     
        ...
        activeclassvalue: function(active,vid){ 
          var gruopname = this._slidercollection(),
              result = new Array();
               var final_array = [];
            ...
            //     var gid = value.replace("_"," ")   
            //       if(_.contains(gruopname['Other'], gid)){
            //       var array = gruopname['Other'][gid]
            //              result.push(...array);
            //     }
            // });
            // for(var i = 0; i < result.length; i++)
            // {
            //   if(result[i].GroupName == vid)
            //   {
            ...
            // }
            // console.log(result)
            return result.length > 0 ? result : gruopname['Addition']["Tops"]
        },
