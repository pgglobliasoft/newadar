...
class Save extends \Sttl\Brand\Controller\Adminhtml\Brand
{
            ...
            try {
                ...
                if (isset($_FILES['small_image']['name']) && $_FILES['small_image']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        $path = $this->_objectManager->get('Magento\Framework\Filesystem')->getDirectoryRead(DirectoryList::MEDIA)
                            ->getAbsolutePath('sttl_brand/');
                        $uploader->save($path);
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['brand']['small_image'] = 'sttl_brand' . $fileName;
                        }
                    } else {
                ...
                if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        ...
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['brand']['image'] = 'sttl_brand' . $fileName;
                        }
                    } else {
                        $this->messageManager->addError(__('Disallowed file type.'));
                        return $this->redirectToEdit($model, $data);
                ...
                if (!isset($data['brand']['url_key']) || $data['brand']['url_key'] == '') {
                    $data['brand']['url_key'] = $data['brand']['name'];
                    //$this->_objectManager->get('Magento\Catalog\Model\Product\Url')->formatUrlKey($data['brand']['name']);
                }
                ...
                $model->save();
                $brand = $this->_objectManager->create('Sttl\Brand\Model\Brand')->load($model->getId());
                $optionId = $this->saveOption('sttl_brand', $brand->getName(), (int)$brand->getOptionId());
                $brand->setOptionId($optionId);
                $brand->save();
                if (isset($data['product_ids']) && ($data['product_ids'] != '' || $data['product_ids'] != null)) {
                    $productIdsInBrand = array();
                    $productCollection = $this->_objectManager->create('Sttl\Brand\Model\Product')->getCollection();
                    $productCollection->addFieldToFilter('brand_id', ['eq' => $brand->getId()]);
                    foreach ($productCollection as $product) {
                        $productIdsInBrand[] = (int)$product->getProductId();
						$product->delete();
                    }
                    $arrAttributeEmpty = ['sttl_brand'=>''];
					...
					if(count($productIdsInput)>0){
						...
						$arrAttributeData = ['sttl_brand'=>$optionId];
						foreach($productIdsInput as $productId){
							$brandProduct = $this->_objectManager->create('Sttl\Brand\Model\Product');
    ...
    {
        return $this->_authorization->isAllowed('Sttl_Brand::save_brand');
    }
