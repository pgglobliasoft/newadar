...
class InstallSchema implements InstallSchemaInterface {
    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context) {
        ...
        $table = $installer->getConnection()
                ->newTable($installer->getTable('sttl_brand'))
                ->addColumn(
        ...
        $table = $installer->getConnection()
                ->newTable($installer->getTable('sttl_brand_product'))
                ->addColumn(
        ...
        $installer->getConnection()->createTable($table);
        $table = $installer->getConnection()
                ->newTable($installer->getTable('sttl_brand_store'))
                ->addColumn(
                ...
                ->addIndex(
                        $setup->getIdxName('sttl_brand_store', ['store_id']), ['store_id']
                )
                ->addForeignKey(
                        $setup->getFkName('sttl_brand_store', 'brand_id', 'sttl_brand', 'brand_id'), 'brand_id', $setup->getTable('sttl_brand'), 'brand_id', \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
                )
                ->addForeignKey(
                        $setup->getFkName('sttl_brand_store', 'store_id', 'store', 'store_id'), 'store_id', $setup->getTable('store'), 'store_id', \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
                )
