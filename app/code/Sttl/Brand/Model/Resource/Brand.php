...
class Brand extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('sttl_brand', 'brand_id');
    }

    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        $condition = ['brand_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('sttl_brand_product'), $condition);
        $this->getConnection()->delete($this->getTable('sttl_brand_store'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('sttl_brand_store');
		...
		if($object->getProductIds()){
            ...
            $this->getConnection()->delete($this->getTable('sttl_brand_product'), $where);
			...
			if(count($productIds)>0){
				...
				$this->getConnection()->insertMultiple($this->getTable('sttl_brand_product'), $productData);
			}
		}
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['sttl_brand_store' => $this->getTable('sttl_brand_store')],
                $this->getMainTable() . '.brand_id = sttl_brand_store.brand_id',
                []
            )->where(
                'status = ?',
                1
            )->where(
                'sttl_brand_store.store_id IN (?)',
                $storeIds
            )->order(
                'sttl_brand_store.store_id DESC'
            )->limit(
                1
            );
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('sttl_brand_store')],
    ...
    public function lookupStoreIds($brandId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('sttl_brand_store'),
            'store_id'
        )->where(
            'brand_id = ?',
            (int)$brandId
    ...
    public function lookupProductIds($brandId)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('sttl_brand_product'),
            'product_id'
        )->where(
            'brand_id = ?',
            (int)$brandId
